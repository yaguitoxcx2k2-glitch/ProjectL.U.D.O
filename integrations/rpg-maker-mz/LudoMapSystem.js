// LudoMapSystem.js v0.11.6 | RPG Maker MZ
/*:
 * @target MZ
 * @plugindesc LUDO Map Editor - mapas e Camadas Visuais animadas v0.11.6-runtime-v5.
 * @author LUDO
 ...
 * @type boolean
 * @default true
 * @command SetVariation
 * @text Trocar variação do mapa
 * @desc Vai para outro estado do mesmo cenário exportado pelo LUDO, mantendo a posição atual do jogador.
 * @arg variation
 * @text Variação
 * @desc Nome mostrado no painel Mapas. Use Base para voltar ao cenário original.
 * @type string
 * @default Base
 * @arg fade
 * @text Transição
 ...
 * @help
 * LudoMapSystem v0.11.6-runtime-v5 - Guia de Ajuda
 *
 * PARA QUE SERVE
 ...
 *
 * CAMADAS VISUAIS
 * Movimento, repeticao, animacao e efeitos sao configurados no Inspector do
 * LUDO. Nao e preciso criar eventos, escrever scripts ou adicionar outro
 * plugin de filtro. Ao reexportar o mapa, este plugin le tudo automaticamente.
 * Os efeitos sao aplicados somente na camada escolhida.
 *
 * ANTES DE COMECAR
 ...
 * Geral:
 * - SetPlayerLevel
 * - SetVariation: troca entre estados do mesmo cenário mantendo a posição.
 *
...
(() => {
    ...
    const fadeOpacity = Math.max(0,Math.min(255,Number(parameters.UnderpassOpacity ?? 150)))/255;
    ...
    function variationTarget(value) {
        ...
        if (Number.isFinite(numeric) && numeric > 0) {
            return manifest.variations.find(item => Number(item && item.mapId) === numeric) || null;
        }
        const key = normalizeVariationName(raw);
        const wantsBase = key === "base" || key === "original" || key === "principal";
        if (wantsBase) return manifest.variations.find(item => item && item.base) || null;
        return manifest.variations.find(item => item && normalizeVariationName(item.name) === key) || null;
    }
    ...
    PluginManager.registerCommand(PLUGIN_NAME,"SetPlayerLevel", args => {
        if (!$gamePlayer) return;
        const level = Number(args.level) === 1 ? 1 : 0;
        // Logical level only; perspective is disabled in this version.
        $gamePlayer._ludoPerspectiveHeight = 0;
        $gamePlayer._ludoDepthLevel = level;
        if (args.followers !== "false")
            for (const follower of ludoFollowers($gamePlayer)) follower._ludoDepthLevel = level;
    });
    PluginManager.registerCommand(PLUGIN_NAME,"SetPerspective", args => {
        if (!$gamePlayer) return;
        $gamePlayer._ludoPerspectiveHeight = 0; // Legacy command: harmless in old events.
    });
    PluginManager.registerCommand(PLUGIN_NAME,"SetVariation", args => {
        setMapVariation(args.variation, args.fade);
    });
    const state = {
        requestedMapId: 0,
        manifestMapId: 0,
    ...
    function validateManifest(raw, mapId) {
        ...
        for (const item of raw.parallaxLayers) {
            ...
            item.oscillationX = Math.max(0, Math.min(4096, Number(item.oscillationX || 0)));
            item.oscillationY = Math.max(0, Math.min(4096, Number(item.oscillationY || 0)));
            item.oscillationSpeed = Math.max(0, Math.min(20, Number(item.oscillationSpeed || 0)));
            item.effect = ["underwater","mist","dream","heat","ghost","night"].includes(String(item.effect || ""))
                ? String(item.effect) : "none";
            item.effectStrength = Math.max(0, Math.min(1, Number(item.effectStrength == null ? 1 : item.effectStrength)));
            item.effectSpeed = Math.max(0, Math.min(10, Number(item.effectSpeed == null ? 1 : item.effectSpeed)));
            item.smoothMotion = item.smoothMotion !== false;
            if (item.animation && typeof item.animation === "object") {
                const animation = item.animation;
                animation.frameWidth = Math.max(1, Math.min(16384, Number(animation.frameWidth || 1)));
                animation.frameHeight = Math.max(1, Math.min(16384, Number(animation.frameHeight || 1)));
                animation.frameCount = Math.max(1, Math.min(4096, Number(animation.frameCount || 1)));
                animation.sheetColumns = Math.max(1, Math.min(4096, Number(animation.sheetColumns || animation.frameCount)));
                animation.fps = Math.max(.1, Math.min(60, Number(animation.fps || 8)));
                animation.pingPong = !!animation.pingPong;
            } else item.animation = null;
        }
    ...
    // Contrato de câmera único: quando LudoCameraSystem estiver ativo usamos
    // seu estado publicado; sem ele, o zoom nativo do MZ continua funcionando.
    // width/height estão no espaço local do mapa, antes do zoom do Spriteset.
    function cameraView() {
        const camera = globalThis.LudoCameraSystem;
        if (camera && typeof camera.renderState === "function") {
            const view = camera.renderState();
            if (view && Number.isFinite(Number(view.zoom))) return view;
        }
        const zoom = Math.max(0.1, Number(typeof $gameScreen !== "undefined" && $gameScreen && $gameScreen.zoomScale
            ? $gameScreen.zoomScale() : 1) || 1);
        return {
            zoom,
            displayPixelX: scrollPixelX(),
            displayPixelY: scrollPixelY(),
    ...
    const PARALLAX_EFFECT_FRAGMENT = `
        ...
        void main(void) {
            ...
            float wave = sin((uv.y * 26.0) + uTime * 2.2) * 0.008 * uStrength;
            if (uMode < 1.5) uv.x += wave;
            else if (uMode > 3.5 && uMode < 4.5) uv.y += sin((uv.x * 34.0) + uTime * 3.4) * 0.006 * uStrength;
            else if (uMode > 2.5 && uMode < 3.5) uv.x += sin((uv.y * 18.0) + uTime) * 0.004 * uStrength;
            vec4 color = texture2D(uSampler, uv);
            if (uMode < 1.5) color.rgb = mix(color.rgb, vec3(0.12, 0.55, 0.76), 0.38 * uStrength);
            else if (uMode < 2.5) {
                float mist = (sin((uv.x + uv.y) * 15.0 + uTime * .7) + 1.0) * .5;
                color.rgb = mix(color.rgb, vec3(.86, .92, .94), (.18 + mist * .12) * uStrength);
            } else if (uMode < 3.5) color.rgb = mix(color.rgb, vec3(.72, .48, .88), .28 * uStrength);
            else if (uMode < 4.5) color.rgb = mix(color.rgb, vec3(1.0, .43, .18), .20 * uStrength);
            else if (uMode < 5.5) {
                color.rgb = mix(color.rgb, vec3(.38, .95, .88), .42 * uStrength);
                color.a *= 1.0 - .22 * uStrength;
            } else color.rgb = mix(color.rgb * vec3(.48, .58, .90), vec3(.05, .08, .20), .26 * uStrength);
            gl_FragColor = color;
        }`;

    function parallaxEffectFilter(item) {
        if (typeof PIXI === "undefined" || typeof PIXI.Filter !== "function" || item.effect === "none") return null;
        const modes={underwater:1,mist:2,dream:3,heat:4,ghost:5,night:6};
        const mode=Number(modes[String(item.effect||"")]||0);if(!mode)return null;
        try{return new PIXI.Filter(null,PARALLAX_EFFECT_FRAGMENT,{uTime:0,uStrength:Number(item.effectStrength||0),uMode:mode});}
        catch(error){console.warn("[LUDO] Efeito visual desativado nesta máquina:",error);return null;}
    }

    function parallaxFrameIndex(item,elapsedMs){
        const animation=item.animation;if(!animation)return 0;
        const count=Math.max(1,Number(animation.frameCount||1));
        const sequence=animation.pingPong&&count>1?count*2-2:count;
        let frame=Math.floor(elapsedMs*Math.max(.1,Number(animation.fps||8))/1000)%sequence;
        if(animation.pingPong&&frame>=count)frame=sequence-frame;
        return Math.max(0,Math.min(count-1,frame));
    }

    function applyParallaxMotionSmoothing(sprite,item){
        if(!sprite||!item)return;
        const smooth=item.smoothMotion!==false;
        sprite.roundPixels=!smooth;
        if(typeof PIXI==="undefined"||!PIXI.SCALE_MODES)return;
        const mode=smooth?PIXI.SCALE_MODES.LINEAR:PIXI.SCALE_MODES.NEAREST;
        const bitmap=sprite._ludoBitmap;
        const bitmapTexture=bitmap&&(bitmap._baseTexture||bitmap.baseTexture);
        if(bitmapTexture)bitmapTexture.scaleMode=mode;
        for(const child of sprite._ludoRepeatChildren||[]){
            child.roundPixels=!smooth;
            if(child.texture&&child.texture.baseTexture)child.texture.baseTexture.scaleMode=mode;
        }
    }

    function rebuildParallaxChildren(sprite,item,w,h,sx,sy){
        const view=cameraView();
        const radiusX=item.repeatX?Math.min(12,Math.ceil(view.viewportWidth/Math.max(1,w*sx))+2):0;
        const radiusY=item.repeatY?Math.min(12,Math.ceil(view.viewportHeight/Math.max(1,h*sy))+2):0;
        const key=radiusX+":"+radiusY+":"+w+":"+h;if(sprite._ludoRepeatKey===key)return;
        for(const old of sprite._ludoRepeatChildren||[])sprite.removeChild(old);
        sprite._ludoRepeatChildren=[];sprite._ludoRepeatKey=key;
        for(let ry=-radiusY;ry<=radiusY;ry++)for(let rx=-radiusX;rx<=radiusX;rx++){
            const child=new Sprite(sprite._ludoBitmap);child.anchor.set(0,0);child.blendMode=blendModeOf(item.blendMode);
            child._ludoRepeatX=rx;child._ludoRepeatY=ry;sprite.addChild(child);sprite._ludoRepeatChildren.push(child);
        }
        applyParallaxMotionSmoothing(sprite,item);
        sprite._ludoFrame=-1;
    }
    ...
    function createParallaxSprite(item,manifest){
        ...
        sprite._ludoRepeatChildren=[];sprite._ludoRepeatKey="";sprite._ludoFrame=-1;
        ...
        sprite.opacity=Math.round(Math.max(0,Math.min(1,Number(item.opacity==null?1:item.opacity)))*255);
        sprite.roundPixels=item.smoothMotion===false;
        if(bitmap&&typeof bitmap.addLoadListener==="function")bitmap.addLoadListener(()=>applyParallaxMotionSmoothing(sprite,item));
        const filter=parallaxEffectFilter(item);if(filter){sprite._ludoEffectFilter=filter;sprite.filters=[filter];}
        return sprite;
    }
    ...
    function updateParallaxSprite(sprite,manifest,elapsedMs){
        ...
        const bitmap=sprite._ludoBitmap,animation=item.animation;
        const w=Math.max(0,Number(animation&&animation.frameWidth||bitmap&&bitmap.width||0));
        const h=Math.max(0,Number(animation&&animation.frameHeight||bitmap&&bitmap.height||0));
        rebuildParallaxChildren(sprite,item,w,h,sx,sy);
        const frame=parallaxFrameIndex(item,elapsedMs);
        if(frame!==sprite._ludoFrame){
            const columns=Math.max(1,Number(animation&&animation.sheetColumns||1));
            for(const child of sprite._ludoRepeatChildren||[]){
                if(animation)child.setFrame((frame%columns)*w,Math.floor(frame/columns)*h,w,h);
                child.x=child._ludoRepeatX*w;child.y=child._ludoRepeatY*h;
            }
            sprite._ludoFrame=frame;
        }
        sprite.scale.set(sx,sy);
        const seconds=elapsedMs/1000,phase=seconds*Number(item.oscillationSpeed||0)*Math.PI*2;
        const dx=Number(view.displayPixelX||0),dy=Number(view.displayPixelY||0);
        // The camera owns travel; zoom and window size only affect framing.
        // Older/native cameras retain the conventional display-based behavior.
        const travelX=Number.isFinite(view.parallaxTravelX)?view.parallaxTravelX:dx;
        const travelY=Number.isFinite(view.parallaxTravelY)?view.parallaxTravelY:dy;
        let x=Number(item.x||0)+(travelX*(1-fx)-dx)/sx+Number(item.speedX||0)*seconds+Math.sin(phase)*Number(item.oscillationX||0);
        let y=Number(item.y||0)+(travelY*(1-fy)-dy)/sy+Number(item.speedY||0)*seconds+Math.cos(phase)*Number(item.oscillationY||0);
        ...
        if(sprite._ludoEffectFilter){
            sprite._ludoEffectFilter.uniforms.uTime=seconds*Number(item.effectSpeed||1);
            if(typeof PIXI!=="undefined"&&typeof PIXI.Rectangle==="function")
                sprite.filterArea=new PIXI.Rectangle(0,0,Graphics.width,Graphics.height);
        }
    }
    ...
    // O MapXXX usa um panorama apenas para o editor nativo do RPG Maker MZ.
    ...
    Spriteset_Map.prototype.updateLudoMapLayers = function() {
        ...
        if(this._ludoParallaxBackground)this._ludoParallaxBackground.scale.set(view.viewportWidth+2,view.viewportHeight+2);
    ...
    // Chamado pelo sistema de câmera logo após ele alterar zoom/displayX/Y.
    // Elimina o quadro de atraso que deslocava Camadas Visuais durante zoom,
    // foco suave, arrasto e transições de câmera.
    Spriteset_Map.prototype.syncLudoCameraTransform = function() {
        this.updateLudoMapLayers();
    };
    ...
    function regionLevelAt(x,y,d=0) {
        ...
        const low = Number(parameters.Level0Region ?? 20), high = Number(parameters.Level1Region ?? 21);
    ...
    function projectSprite(sprite, level, baseZ) {
        ...
        if (sprite._ludoItem) {
            const base = Math.max(0,Math.min(1,Number(sprite._ludoItem.opacity ?? 1)));
    ...
    // API publica somente-leitura para sistemas oficiais (Reflection etc.).
    ...
    globalThis.LudoMapSystem = LudoMapSystemAPI;
