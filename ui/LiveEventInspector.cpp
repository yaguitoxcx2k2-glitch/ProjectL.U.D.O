...
LiveEventInspector::LiveEventInspector(game::GameSession& session, QWidget* parent)
    : QDockWidget(tr("Live Event Inspector"), parent), m_session(session)
{
    ...
    auto* executionButtons=new QHBoxLayout;auto* pause=new QPushButton(tr("Pausar"),page);auto* continueRun=new QPushButton(tr("Continuar"),page);auto* stepInto=new QPushButton(tr("Step Into"),page);auto* stepOver=new QPushButton(tr("Step Over"),page);auto* stepOut=new QPushButton(tr("Step Out"),page);
    ...
    m_stack=new QTableWidget(page);m_stack->setColumnCount(4);m_stack->setHorizontalHeaderLabels({tr("Nível"),tr("Fonte"),tr("Evento"),tr("Comando")});
    ...
    auto* watchButtons=new QHBoxLayout;auto* add=new QPushButton(tr("+ Watch"),page);auto* remove=new QPushButton(tr("Remover"),page);
    ...
    m_watches=new QTableWidget(page);m_watches->setColumnCount(6);m_watches->setHorizontalHeaderLabels({tr("Tipo"),tr("Nome / Referência"),tr("Valor"),tr("Mudou"),tr("Break"),tr("Condição")});
    ...
    auto* filterRow=new QHBoxLayout;m_filter=new QLineEdit(page);m_filter->setClearButtonEnabled(true);m_filter->setPlaceholderText(tr("Filtrar trace por fase, fonte, evento ou comando…"));
    auto* exportText=new QPushButton(tr("Exportar TXT"),page);auto* exportJson=new QPushButton(tr("Exportar JSON"),page);filterRow->addWidget(m_filter,1);filterRow->addWidget(exportText);filterRow->addWidget(exportJson);root->addLayout(filterRow);
    m_trace=new QTableWidget(page);m_trace->setColumnCount(6);m_trace->setHorizontalHeaderLabels({tr("#"),tr("Fase"),tr("Fonte"),tr("Evento"),tr("Comando"),tr("Detalhes")});
    ...
    auto* playbackRow=new QHBoxLayout;m_playbackLabel=new QLabel(tr("Playback: sem trace"),page);m_playback=new QSlider(Qt::Horizontal,page);m_playback->setRange(0,0);playbackRow->addWidget(m_playbackLabel);playbackRow->addWidget(m_playback,1);root->addLayout(playbackRow);
    setWidget(page);
    ...
    connect(m_playback,&QSlider::valueChanged,this,[this](int value){if(value>=0&&value<m_trace->rowCount()){m_trace->selectRow(value);m_trace->scrollToItem(m_trace->item(value,0));m_playbackLabel->setText(tr("Playback: %1 / %2").arg(value+1).arg(m_trace->rowCount()));}});
...
{
    bool ok=false;const QStringList kinds={tr("Variável"),tr("Switch"),tr("String"),tr("Game Value")};
    const QString chosen=QInputDialog::getItem(this,tr("Adicionar watch"),tr("Tipo:"),kinds,0,false,&ok);if(!ok)return;
    ...
    if(labels.isEmpty())return;const QString selected=QInputDialog::getItem(this,tr("Adicionar watch"),tr("Valor monitorado:"),labels,0,false,&ok);if(!ok)return;
...
void LiveEventInspector::configureWatchCondition()
{
    ...
    const QString op=QInputDialog::getItem(this,tr("Condição do watch"),tr("Pausar quando:"),ops,0,false,&ok);if(!ok)return;
    const QString value=QInputDialog::getText(this,tr("Condição do watch"),tr("Valor de comparação:"),QLineEdit::Normal,QString(),&ok);if(!ok)return;
    m_session.setDebugWatchCondition(serial,op,typedInput(value));refresh();
}

void LiveEventInspector::exportTrace(bool json)
{
    const QString path=QFileDialog::getSaveFileName(this,json?tr("Exportar trace JSON"):tr("Exportar trace TXT"),json?QStringLiteral("ludo-event-trace.json"):QStringLiteral("ludo-event-trace.txt"),json?tr("JSON (*.json)"):tr("Texto (*.txt)"));if(path.isEmpty())return;
...
void LiveEventInspector::refresh()
{
    ...
    m_runtime->setText(tr("Cutscene depth: <b>%1</b> · Awaitable: <b>%2</b> · Cancelamento: <b>%3</b><br>Parallel tickets: %4<br>Último alvo: %5<br>Última condição: %6")
    ...
    const QSignalBlocker blocker(m_playback);const int old=m_playback->value();m_playback->setRange(0,qMax(0,m_trace->rowCount()-1));m_playback->setValue(qBound(0,old,m_playback->maximum()));m_playbackLabel->setText(m_trace->rowCount()?tr("Playback: %1 / %2").arg(m_playback->value()+1).arg(m_trace->rowCount()):tr("Playback: sem trace"));
}
